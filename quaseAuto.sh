...
echo -e "${YELLOW}--- Iniciando a automação de setup (sem clonagem de repositórios) ---${NC}"

# --- Limpeza de diretórios de configuração existentes ---
echo -e "${RED}1. Verificando e removendo diretórios de configuração para garantir uma instalação limpa...${NC}"
rm -rf nginx cloudflare
echo -e "${GREEN}Diretórios limpos.${NC}"

# --- Criando pastas para Nginx e Cloudflare ---
echo -e "${GREEN}2. Criando pastas para Nginx e Cloudflare...${NC}"
mkdir -p nginx
mkdir -p cloudflare

# --- Criando arquivos de configuração iniciais ---
echo -e "${GREEN}3. Criando arquivos .env e de configuração com as suas configurações...${NC}"
# Arquivo .env do Chatwoot
cat << EOF > chatwoot/.env
...
cat << EOF > evolution/.env
# Aprenda sobre as diversas variáveis de ambiente em
# https://github.com/EvolutionAPI/evolution-api/blob/main/docs/pt-BR/EnvironmentVariables.md

# Servidor de API - http ou https
SERVER_TYPE=http
...
server {
    listen 80;
    server_name chat.mordorlab.com;
...
server {
    listen 80;
    server_name api.mordorlab.com;

    location / {
        proxy_pass http://evolution:3000;
        proxy_set_header Host \$host;
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto \$scheme;
...
server {
    listen 80;
    server_name host.mordorlab.com;

    location / {
        return 200 'OK - Host Mordorlab';
        add_header Content-Type text/plain;
    }
}

# Futura configuração para n8n
# server {
#     listen 80;
#     server_name n8n.mordorlab.com;
#     location / {
#         proxy_pass http://n8n_service:5678;
#         ...
#     }
# }
EOF

# Arquivo de configuração do Cloudflare Tunnel
cat << EOF > cloudflare/config.yml
# Você precisa criar um túnel na sua conta Cloudflare e preencher aqui
tunnel: a45f1685-ae04-443b-a29b-b678624f4006
credentials-file: /etc/cloudflared/a45f1685-ae04-443b-a29b-b678624f4006.json

# Rotas
ingress:
  - hostname: chat.mordorlab.com
    service: http://nginx:80
  - hostname: api.mordorlab.com
    service: http://nginx:80
  - hostname: host.mordorlab.com
    service: http://nginx:80
  - hostname: n8n.mordorlab.com
    service: http://nginx:80
  - service: http_status:404
EOF

echo -e "${YELLOW}--- Setup inicial concluído. Preencha os arquivos .env antes de prosseguir. ---${NC}"

# --- Prepara o banco de dados do Chatwoot ---
echo -e "${GREEN}4. Agora, vamos preparar o banco de dados do Chatwoot. Isso levará alguns minutos.${NC}"
echo -e "${YELLOW}Primeiro, vamos subir os serviços de banco de dados...${NC}"
# Garante que o arquivo tenha as permissões corretas
chmod +r docker-compose.yaml
docker-compose -f docker-compose.yaml up -d postgres redis

# Etapa adicionada: Instalar dependências (gems) antes de preparar o banco de dados
echo -e "${YELLOW}5. Instalando dependências (gems) do Chatwoot...${NC}"
docker-compose -f docker-compose.yaml run --rm chatwoot-rails bundle install

echo -e "${YELLOW}6. Executando o comando de preparação do banco de dados do Chatwoot...${NC}"
docker-compose -f docker-compose.yaml run --rm chatwoot-rails bundle exec rails db:chatwoot_prepare

echo -e "${YELLOW}--- Setup completo! Execute 'docker-compose up -d' para iniciar os serviços. ---${NC}"
